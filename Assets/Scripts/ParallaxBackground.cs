...
public class ParallaxBackground : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        //check to see if the camera is going 'left'
    ...
    // Function that moves the position of the farthest right background layer image to the be the farthest left image 
    ...
    private void goLeft()
    {
        ...
        // move the right image panel to the right side of the 'last' left image panel
        layers[rightIndex].position = Vector3.right * (layers[leftIndex].position.x - backgroundSize);

        // set the left image panel to that newly moved image panel
        leftIndex = rightIndex;

        // RightIndex is now the preious image panel in the list
        rightIndex--;

        // fix if right index goes out of bounds this means the index is the last in the array
        if (rightIndex < 0)
            rightIndex = layers.Length - 1;
    }
